...
#define VECTOR_TYPE(TYPE) struct { \
    TYPE* data; \
    size_t length; \
    size_t capacity; \
}
...
#define VECTOR_ENSURE_LENGTH(VEC, LENGTH) do { \
    ...
    if ((LENGTH) >= VECTOR_ENSURE_LENGTH_vec->capacity) { \
        ...
        VECTOR_ENSURE_LENGTH_vec->data = krealloc(VECTOR_ENSURE_LENGTH_vec->data, VECTOR_ENSURE_LENGTH_vec->capacity * sizeof(*VECTOR_ENSURE_LENGTH_vec->data)); \
    } \
} while (0)
...
#define VECTOR_FOREACH(VEC, BINDING, ...) do { \
