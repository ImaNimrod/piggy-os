...
void virtio_net_init(struct virtio_device* vio_dev) {
    ...
    if (unlikely(!isr_allocate_vector(&vector))) {
        kpanic(NULL, false, "failed to allocate IRQ vector for VirtIO network device receive queue");
    }

    if (!virtio_queue_init(vio_dev, 0, vector)) {
        klog("[virtio_net] failed to initialize VirtIO network device receive queue\n");
        return;
    }
    ...
    if (unlikely(!isr_allocate_vector(&vector))) {
        kpanic(NULL, false, "failed to allocate IRQ vector for VirtIO network device transmit queue");
    }
