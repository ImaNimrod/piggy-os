...
void puts(const char* str) {
    syscall3(SYS_WRITE, stdout, (uint64_t) str, strlen(str));
}
...
void exec(const char* path, const char** argv, const char** envp) {
    syscall3(SYS_EXEC, (uint64_t) path, (uint64_t) argv, (uint64_t) envp);
}

int wait(int child_pid) {
    return syscall1(SYS_WAIT, child_pid);
}
