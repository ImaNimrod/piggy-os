...
__attribute__((noreturn)) void kpanic(struct registers* r, bool trace_stack, const char* fmt, ...) {
    ...
    if (cmdline_early_get_klog()) {
        ...
        if (r != NULL) {
            ...
            printf("CR0: 0x%016x CR2: 0x%016x CR3: 0x%016x CR4: 0x%016x\n", read_cr0(), read_cr2(), read_cr3(), read_cr4());
            printf("RIP: 0x%016x RFLAGS: 0x%016x CS: 0x%04x SS: 0x%04x ERROR CODE: 0x%08x", r->rip, r->rflags, r->cs, r->ss, r->error_code);
        }
